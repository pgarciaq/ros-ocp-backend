...
lint: golangci-lint
	...
	# Nested module is not part of parent ./...
	cd librobne && $(GOLANGCILINT) run --timeout=3m --config=../.golangci.yml ./...

.PHONY: test
RACE ?=
test: ## Run all tests (RACE=1 to enable the race detector)
	# Nested module is not part of parent ./...
	go test -C librobne $(if $(RACE),-race) -count=1 ./...
...
test-short: ## Run unit tests only (skips Docker/testcontainers integration tests)
	go test -C librobne -short -count=1 ./...
