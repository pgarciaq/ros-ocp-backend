...
func RecommendVM(
	...
	if isAbandoned {
		...
		cpuMarginBP = int32(cpuMargin * float64(fixedpoint.BasisPointsScale))
		memMarginBP = int32(cfg.MemMarginMin * float64(fixedpoint.BasisPointsScale))
...
func vmDiskProjectionHypervisor(days []Digest, cfg VMRecConfig) (
	...
	growthPerDay := node.LinearRegressionSlope(series)
...
func VMExplFromRecommendation(r Recommendation) types.VMExplanationFactors {
	var dataDays int
	if r.ExplDataDays != nil {
		dataDays = *r.ExplDataDays
	}
	return types.VMExplanationFactors{
		DataDays:               dataDays,
