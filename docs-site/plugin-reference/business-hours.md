...
**Optimizations UI (koku-ui-ros, [#493](https://github.com/pgarciaq/ros-ocp-backend/issues/493) / [#494](https://github.com/pgarciaq/ros-ocp-backend/issues/494)):** Peak hours cards/columns on node, GPU MIG, timeslicing, and VM **detail** only. Lists stay all-hours. Hide reason-only nests. Warning copy is the nest `message` for **79–82**. Do not reuse container YAML Peak hours. Timeslicing detail is `GET .../gpu/timeslicing/{node}`. MIG extra-fetches container detail `gpu.{term}` until list rows include `id` ([#495](https://github.com/pgarciaq/ros-ocp-backend/issues/495)). Visual Insights Peak hours charts: node/VM sibling `daily_digests_business_hours` (BH rec on that series only); MIG dual radar from the GPU nest; timeslicing radar from nest SM/VRAM. Hide Peak hours charts when the nest is reason-only.
