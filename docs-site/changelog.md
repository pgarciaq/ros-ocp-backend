...
- **Namespace ingest parse onto `librobne/csv` ([#501](https://github.com/pgarciaq/ros-ocp-backend/issues/501), namespace cut):**
  ...
  throttle / min / RSS-max and hard-quota copied from `*_sum`. No API change.

- **PVC ingest parse onto `librobne/csv` ([#501](https://github.com/pgarciaq/ros-ocp-backend/issues/501), PVC cut):**
  Processor storage CSV streaming uses `librobne/csv.ForEachPVC` (one parse
...
- **robne CLI BH notification codes 79–82 ([#492](https://github.com/pgarciaq/ros-ocp-backend/issues/492)):**
  ...
  **79** (node), **80** (GPU container), **81** (GPU timeslicing), or **82** (VM)
  ...
  Envelope stays **11**. Empty BH arrays stay `[]` with no codes.
...
- **robne CLI business-hours JSON siblings ([#487](https://github.com/pgarciaq/ros-ocp-backend/issues/487)):**
  ...
  **10**. CLI siblings are full DTOs — **not** the product thin nest.
  Dual-write node/GPU/VM BH **digests**; do not upsert BH recs.
  `explain --schedule business_hours` is unlocked for node/gpu/timeslicing/vm.
...
- **robne CLI binary identity / envelope capability ([#489](https://github.com/pgarciaq/ros-ocp-backend/issues/489)):**
  ...
  `business_hours`=10, `business_hours_plugins`=11). JSON recommend `"version"` stays per-run (ADR-0336);
...
- **Per-PVC VM shared storage detection
  ...
  child table (migration 000180). See [ADR-0324](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0324-vm-pvc-companion-csv-for-shared-storage-detection.md).
...
- **Calendar-accurate monthly hours for savings extrapolation
  ...
  [ADR-0182](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0182-monthly-savings-730-hours.md);
  see [ADR-0326](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0326-calendar-accurate-monthly-hours.md).
...
- **Graduated production security enforcement ([#168](https://github.com/pgarciaq/ros-ocp-backend/issues/168)):**
  ...
  See [`docs/operations/security-enforcement.md`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/operations/security-enforcement.md).
...
- **Replica count optimization (Phase 1):** New recommendation type that suggests
  ...
  [ADR-0309](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0309-replica-count-optimization-phase1.md))
...
- **Auto-detect SaaS mode for heavy API statement timeout:** When Clowder is
  ...
  [ADR-0308](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0308-auto-lower-heavy-api-timeout-saas.md))
...
- **`filter[stale]` correctness and performance:** The `filter[stale]=true` and
  ...
  [ADR 0306](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0306-stale-filter-org-container-keys-column.md))
...
- **OOM timeline endpoint:** New
  ...
  [ADR-0302](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0302-oom-timeline-endpoint.md))
...
- **COST-7274:** Removed fixed six-type `workload_type` allowlist from the API and
  ...
  ([ADR-0300](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0300-remove-fixed-workload-type-allowlist.md))
...
- Typed `expl_*` explanation columns for all native-engine recommendation types ([ADR-0296](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0296-recommendation-explanation-factors-typed-columns.md))
...
- [ADR-0296](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0296-recommendation-explanation-factors-typed-columns.md): Store recommendation explanation factors as typed columns (persist engine intermediate values at write time; expose via detail API `explanation` object)
- [ADR-0297](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0297-gpu-time-slicing-recommendation-persistence.md): GPU time-slicing recommendation persistence at ingest
- Implementation plan: [`docs/plans/recommendation-explanations.md`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/plans/recommendation-explanations.md) covering container, namespace, node, GPU, PVC, quota, cluster-quota, and VM recommendation types
- Implementation plan: [`docs/plans/gpu-time-slicing-persistence.md`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/plans/gpu-time-slicing-persistence.md)
- **GPU time-slicing recommendation persistence** ([ADR-0297](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0297-gpu-time-slicing-recommendation-persistence.md)): move node time-slicing from compute-at-read to compute-at-ingest
...
- [ADR-0298](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0298-composite-key-sweep-stale-detection.md): Post-reconcile composite-key sweep (`MarkUnreportedContainersStale`) marks recommendations stale when their composite key no longer matches any current digest data (e.g., `workload_type` change from `deployment` to `statefulset`). Complements the existing cluster-level staleness check from [ADR-0224](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0224-stale-marking-precedence-last-reported-at-overrides-digest-age.md).
- [ADR-0295](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0295-integer-first-architecture.md): Documented the overarching integer-first arithmetic principle across the native engine
...
- `ROS_USE_NATIVE_ENGINE` — removed; native engine is unconditionally active (see [ADR-0157](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/0157-ros-enabled-plugins-replaces-native-flag.md))
...
- Advisory CI workflow [`.github/workflows/openapi-changelog-check.yml`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/.github/workflows/openapi-changelog-check.yml): warns when API-affecting paths (see [`.github/openapi-paths.txt`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/.github/openapi-paths.txt)) change without `openapi.json` updates, or when Go files change without `CHANGELOG.md` updates (finding #53 resolved).
- Advisory CI workflow [`.github/workflows/adr-reminder.yml`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/.github/workflows/adr-reminder.yml): reminds authors to review or create ADRs when architectural paths change (see [`.github/architectural-paths.txt`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/.github/architectural-paths.txt)) (finding #54 resolved).
- `govulncheck` CI workflow [`.github/workflows/govulncheck.yml`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/.github/workflows/govulncheck.yml) on PRs and weekly schedule (finding #60 resolved).
...
- Adversarial due diligence review **v2.0** ([`docs/audits/adversarial-review.md`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/audits/adversarial-review.md)): fresh audit acknowledging v1.6 remediations (#1–#31) and documenting 29 new findings (#32–#60) across ingestion edge cases, GPU fleet-scale performance, auth hardening gaps, and governance.
...
- Architecture Decision Records: 162 ADRs in [`docs/adr/`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/adr/README.md) with index, covering engine, data model, API, ingestion, plugins, cost, tags, deployment, testing, security, Kafka, and configuration decisions (adversarial review finding #30 resolved).
...
- Migration CI lint [`scripts/lint-migrations.sh`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/scripts/lint-migrations.sh), [`docs/operations/large-table-migrations.md`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/operations/large-table-migrations.md), and [`deploy/migrations/concurrent-index-job.yaml`](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/deploy/migrations/concurrent-index-job.yaml) (finding #24 mitigated).
