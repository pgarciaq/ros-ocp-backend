...
|---|---|---|---|
| ros-ocp-backend | `github.com/pgarciaq/ros-ocp-backend` | `pgarciaq-rosocp-superpowers-phase17` | Native recommendation engine (API, processor, poller, housekeeper) |
| koku | `github.com/pgarciaq/koku` | `pgarciaq-rosocp-superpowers-phase17` | Cost Management backend (listener, Masu, effective_rates for savings) |
| koku-ui | `github.com/pgarciaq/koku-ui` | `pgarciaq-rosocp-superpowers-phase17` | Cost Management frontend (optimizations UI with native engine features) |
| koku-metrics-operator | `github.com/pgarciaq/koku-metrics-operator` | `pgarciaq-rosocp-superpowers-phase17` | Cluster metrics collection operator (Prometheus queries, CSV generation) |
| cost-onprem-chart | `github.com/pgarciaq/cost-onprem-chart` | `pgarciaq-rosocp-superpowers-phase17` | Helm chart for on-prem deployment + E2E test suite |
| nise | `github.com/pgarciaq/nise` | `pgarciaq-rosocp-superpowers-phase17` | Test data generator (NISE) |
| costmgmt-api-cheatsheet | `github.com/pgarciaq/costmgmt-api-cheatsheet` | `pgarciaq-rosocp-superpowers-phase17` | API documentation + Bruno collection for manual testing |
...
- All repos must be on the **same branch** (`phase17`) for compatibility. The `phase17` branch builds on top of `phase16` which builds on `phase15` etc. — it is cumulative.
...
!!! warning "Branch alignment is critical"
    All repositories must use the same phase branch. Mixing branches (e.g., phase17
...
**Latest full-suite baseline (phase12 images, June 2026):** **501 passed, 2 failed, 35 skipped** with `--all`. Rebuild and push **new image tags** before every E2E run; never run raw `pytest` without `run-pytest.sh` (venv + env setup). Cluster rebuild/tag/log rules: [AGENTS.md](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/AGENTS.md) and [sno-cluster-operations](https://github.com/pgarciaq/ros-ocp-backend/blob/{{ git_branch }}/docs/agents/sno-cluster-operations.md). Chart pytest still lives in cost-onprem-chart until that harness is replaced.
