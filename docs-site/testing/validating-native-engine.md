...
|---|---|---|---|
| ros-ocp-backend | `github.com/pgarciaq/ros-ocp-backend` | `pgarciaq-rosocp-superpowers-phase16` | Native recommendation engine (API, processor, poller, housekeeper) |
| koku | `github.com/pgarciaq/koku` | `pgarciaq-rosocp-superpowers-phase16` | Cost Management backend (listener, Masu, effective_rates for savings) |
| koku-ui | `github.com/pgarciaq/koku-ui` | `pgarciaq-rosocp-superpowers-phase16` | Cost Management frontend (optimizations UI with native engine features) |
| koku-metrics-operator | `github.com/pgarciaq/koku-metrics-operator` | `pgarciaq-rosocp-superpowers-phase16` | Cluster metrics collection operator (Prometheus queries, CSV generation) |
| cost-onprem-chart | `github.com/pgarciaq/cost-onprem-chart` | `pgarciaq-rosocp-superpowers-phase16` | Helm chart for on-prem deployment + E2E test suite |
| nise | `github.com/pgarciaq/nise` | `pgarciaq-rosocp-superpowers-phase16` | Test data generator (NISE) |
| costmgmt-api-cheatsheet | `github.com/pgarciaq/costmgmt-api-cheatsheet` | `pgarciaq-rosocp-superpowers-phase16` | API documentation + Bruno collection for manual testing |
...
- All repos must be on the **same branch** (`phase16`) for compatibility. The `phase16` branch builds on top of `phase15` which builds on `phase14` etc. — it is cumulative.
...
!!! warning "Branch alignment is critical"
    All repositories must use the same phase branch. Mixing branches (e.g., phase16
...
|---|---|---|
| `ros-ocp-backend` | `pgarciaq-rosocp-superpowers-phase16` | `pgarciaq` |
| `koku-metrics-operator` | `pgarciaq-rosocp-superpowers-phase16` | `pgarciaq` |
| `koku` | `pgarciaq-rosocp-superpowers-phase16` | `pgarciaq` |
...
|---|---|---|
| `ros-ocp-backend` | `pgarciaq-rosocp-superpowers-phase16` | Native engine |
...
| **nise** | `~/dev/koku/nise` | **`pgarciaq-rosocp-superpowers-phase16`** | Synthetic OCP/ROS CSVs (`--ros-ocp-info`, `--write-monthly`) |
...
git -C ros-ocp-backend checkout pgarciaq-rosocp-superpowers-phase16
git -C koku-metrics-operator checkout pgarciaq-rosocp-superpowers-phase16
git -C koku checkout pgarciaq-rosocp-superpowers-phase16
git -C nise checkout pgarciaq-rosocp-superpowers-phase16
...
# (ros-ocp-backend stays on pgarciaq-rosocp-superpowers-phase16)
```
...
cd ~/dev/koku/ros-ocp-backend && git checkout pgarciaq-rosocp-superpowers-phase16
