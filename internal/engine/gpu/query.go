...
func QueryGPURecommendations(ctx context.Context, pool *pgxpool.Pool, orgID, clusterUUID string, start, end time.Time, terms []core.TermConfig, digestFilters *GPUQueryFilters) (map[GPUContainerKey][]*GPURec, map[GPUContainerKey]string, map[string]time.Time, error) {
...
func queueGPUClassificationUpdate(batch *pgx.Batch, w gpuClassificationWrite) {
	batch.Queue(gpuClassificationUpdateSQL,
		...
		}, core.AppendGPUExplArgs(nil, w.expl)...)...,
	)
}
...
func StoreGPUClassifications(ctx context.Context, pool *pgxpool.Pool, orgID, clusterUUID string, terms []core.TermConfig, costData *costdata.ClusterCostData) error {
	now := time.Now().UTC()
	...
	for key, recs := range gpuRecs {
		for _, rec := range recs {
			...
			writes = append(writes, gpuClassificationWrite{
				...
				idleState:        core.IdleStateForWrite(rec.GPUIdleState),
...
func gpuExplFromRec(rec GPURec, fbP98MiB int32) core.GPUExplanationFactors {
	return core.GPUExplanationFactors{
