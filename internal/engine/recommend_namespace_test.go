...
func TestRecommendNamespaces_NoPool(t *testing.T) {
	...
	cfg := DefaultNamespaceEngineConfig("org-1", "cluster-1", now, end)
...
func TestRecommendNamespaces_EmptyAndAllow(t *testing.T) {
	...
	grouped := map[namespaceKey][]DigestRow{
		...
		{Namespace: "keep-me"}: make([]DigestRow, 14),
	}
...
func TestRecommendNamespaces_DoesNotCallTimeNow(t *testing.T) {
	...
	grouped := map[namespaceKey][]DigestRow{{Namespace: "ns"}: rows}
