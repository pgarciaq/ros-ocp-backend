...
func ClassifyNodeIdleState(class nodeClassification, settings ThresholdSettings) core.IdleState {
	if class.validDays == 0 {
		return core.IdleStateActive
	}

	if class.maxCPUUsageP95MC < settings.ZombieCPUP95MC && class.PodCount <= settings.ZombieMaxPods {
		return core.IdleStateZombie
	}
