...
func RecommendNodes(digests []DigestRow, cfg RecConfig, nodeSettings ThresholdSettings, terms []core.TermConfig) []Rec {
	...
	for node, allDays := range grouped {
		...
		for _, tc := range terms {
			...
			confidence := core.ComputeConfidence(dataDays, tc.MinDataDays, tc.WindowDays)
...
func evaluateNodeNotifications(codes []int16, confidence float32, dataDays int) []int16 {
	if confidence < defaultLowConfidenceThreshold && dataDays > 0 {
		codes = core.AppendUnique(codes, core.NotifLowConfidence)
	}
	if dataDays > 0 && dataDays <= defaultSparseDataThreshold {
		codes = core.AppendUnique(codes, core.NotifSparseData)
	}
	return codes
}
...
func classifyNode(node string, days []DigestRow, cfg RecConfig, nodeSettings ThresholdSettings, halfLifeHours float64, endDate time.Time) nodeClassification {
	...
	if class.PodSchedulingHeadroom >= 0 && headroomBP < notificationThBP {
		class.NotificationCodes = append(class.NotificationCodes, core.NotifNodePodSchedulingLimit)
	}
	...
	if avgCPU95BP < cfg.UnderutilThresholdBP && avgMem95BP < cfg.UnderutilThresholdBP {
		...
		class.NotificationCodes = append(class.NotificationCodes, core.NotifNodeUnderutilized)
	}
	...
	if allocCPU > 0 && maxRequests > 0 {
		...
		if ratioBP > cfg.OvercommitThresholdBP {
			...
			class.NotificationCodes = append(class.NotificationCodes, core.NotifNodeOvercommitted)
		}
	}
	...
	if len(imbalances) >= 2 {
		...
		if finalImbalanceBP > imbalanceThreshBP {
			...
			class.NotificationCodes = append(class.NotificationCodes, core.NotifStrandedResources)
		}
	}
...
func nodeExplanationFromClass(class nodeClassification, eng EngineConfig, nodeCountReduction int) core.NodeExplanationFactors {
	formula := "target_util"
	if class.IdleState == core.IdleStateIdle || class.IdleState == core.IdleStateZombie {
		formula = "idle"
	...
	return core.NodeExplanationFactors{
...
func appendFleetConsolidationNotifications(recs []Rec, indices []int, machineSetName string, groupReduction int) {
	...
	for _, i := range indices {
		...
		recs[i].NotificationCodes = core.AppendUnique(recs[i].NotificationCodes, core.NotifNodeFleetConsolidation)
	}
}
...
func minimumNodesForWorkload(totalCPUP95, totalMemP95, nodeCPUMC, nodeMemKiB int64, targetUtilization float64) int64 {
	targetScaled := int64(math.Round(targetUtilization * float64(core.MarginScale)))
	if targetScaled <= 0 {
		targetScaled = int64(0.8 * float64(core.MarginScale))
	}

	var nodesCPU, nodesMem int64 = 1, 1
	if nodeCPUMC > 0 && totalCPUP95 > 0 {
		capacity := nodeCPUMC * targetScaled / core.MarginScale
		if capacity > 0 {
			nodesCPU = ceilDivInt64(totalCPUP95, capacity)
		}
	}
	if nodeMemKiB > 0 && totalMemP95 > 0 {
		capacity := nodeMemKiB * targetScaled / core.MarginScale
		if capacity > 0 {
...
func hasFullSpareNodeHeadroom(currentCPUmc, currentMemKiB, recCPUmc, recMemKiB int64, multiplier float64) bool {
	...
	multScaled := int64(math.Round(multiplier * float64(core.MarginScale)))
	return currentCPUmc*core.MarginScale >= recCPUmc*multScaled && currentMemKiB*core.MarginScale >= recMemKiB*multScaled
}
...
func recommendedNodeCapacity(maxCPUUsageP95MC, maxMemUsageP95KiB, maxCPURequestsMC, maxMemRequestsKiB int64, targetUtilization float64) (cpuMC, memKiB int64) {
	...
	if maxCPUUsageP95MC > 0 {
		recommendedCPUMC = ceilDivInt64(maxCPUUsageP95MC*core.MarginScale, targetScaled)
	}
	if maxCPURequestsMC > 0 {
		requestBased := ceilDivInt64(maxCPURequestsMC*core.MarginScale, targetScaled)
		if requestBased > recommendedCPUMC {
			recommendedCPUMC = requestBased
		}
	}
	if maxMemUsageP95KiB > 0 {
		recommendedMemKiB = ceilDivInt64(maxMemUsageP95KiB*core.MarginScale, targetScaled)
	}
	if maxMemRequestsKiB > 0 {
		requestBased := ceilDivInt64(maxMemRequestsKiB*core.MarginScale, targetScaled)
