...
func initLogger() {
	...
	if cfg.CwAccessKey != "" {
		awsCfg := &aws.Config{
			Region:      aws.String(cfg.CwRegion),
			Credentials: credentials.NewStaticCredentials(cfg.CwAccessKey, cfg.CwSecretKey, ""),
		}
		hook, err := lc.NewBatchingHook(cfg.CwLogGroup, cfg.CwLogStream, awsCfg, 10*time.Second)
		if err != nil {
			logger.Info(err)
		}
