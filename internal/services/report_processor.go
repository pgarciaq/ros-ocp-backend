...
func ProcessReport(ctx context.Context, msg *kafka.Message, consumer *kafka.Consumer) {
	...
	// Parallel file download+ingest bounded by ManifestDownloadWorkers.
	// Transient errors cancel remaining goroutines; permanent errors are recorded per-file.
	fileTransientErr, filePermanentFailed := parallelIngestFiles(ctx, pool, log, kafkaMsg, manifestID, useNativeCSVIngest, &rhAccount, &cluster)
	if fileTransientErr != nil {
		recordKafkaTransient(fileTransientErr)
	}
	if filePermanentFailed {
...
func runNamespaceRecommendations(ctx context.Context, kafkaMsg types.KafkaMsg) error {
	...
	log.Infof("native namespace engine: wrote %d all-hours recommendations", len(results))

	namespaceAnalyticsDegraded, histErr := engine.WriteNamespaceRecommendationHistories(
		ctx, pool, results, isTransientKafkaProcessingError,
	)
	if histErr != nil {
