...
func get_user_permissions_from_rbac(encodedIdentity string) (map[string][]string, error) {
	...
	acls, truncated, err := request_user_access(url, encodedIdentity)
	if err != nil {
		return nil, err
	}
	if len(acls) > 0 {
		...
		if len(permissions) > 0 {
			// #543: never cache a truncated partial. The next request
			// re-pages upstream instead of serving the stale set for the
			// full TTL with no retry and no new signal.
			if cacheEnabled && !truncated {
...
func request_user_access(url, encodedIdentity string) ([]types.RbacData, bool, error) {
	...
	for page := 0; page < maxRBACPages; page++ {
		...
		if err != nil {
			...
			return nil, false, fmt.Errorf("create RBAC request: %w", err)
		}
		...
		if err != nil {
			...
			return nil, false, fmt.Errorf("call RBAC API: %w", err)
		}
		...
		if err := json.Unmarshal(body, &response); err != nil {
			...
			return nil, false, fmt.Errorf("unmarshal RBAC API response: %w", err)
		}
		...
		if !strings.HasPrefix(response.Links.Next, "/api/rbac/") {
			...
			return nil, false, fmt.Errorf("RBAC pagination link has unexpected prefix")
		}
