...
func GetQuotaRecommendations(c echo.Context) error {
	...
	if hasCursor {
		seekSQL, seekArgs, nextIdx, seekErr := quotaSeekSQL(orderCol, orderDir, cursor, cursor.OrderBy != "", argIdx)
		if seekErr != nil {
	...
	if projection.reproject {
		...
		var reprojErr error
		data, reprojErr = applyQuotaListReprojection(ctx, pool, orgID, cfg, projection, data, storedCurrency)
		if reprojErr != nil {
	...
	for i := range data {
		convertAndPatchAmount(data[i].EstimatedSavings, rate, displayCurrency)
	}
...
func getQuotaRecommendationsGrouped(
	...
	for rows.Next() {
		...
		if savingsCents > 0 {
			item.EstimatedSavings = money.FormatCentsToAmountPtr(&savingsCents, storedCurrency)
