...
// ComputeDigest computes exact percentiles from in-memory samples (librobne).
func ComputeDigest(values []int64) Digest {
	return libdigest.ComputeDigest(values)
}

// ComputeWeightedDigest computes percentiles using per-sample weights (librobne).
func ComputeWeightedDigest(values []int64, weights []float64) Digest {
	return libdigest.ComputeWeightedDigest(values, weights)
}

func weightedPercentileFromSorted(sorted []int64, weights []float64, pct float64) int64 {
	return libdigest.WeightedPercentileFromSorted(sorted, weights, pct)
}

const maxCVSpareInner = 32
