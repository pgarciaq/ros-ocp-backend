...
3. **Parse** container ROS via `librobne/csv.ForEachRow` (ingest wrapper `internal/ingestion/csvparser.go`), namespace ROS via `librobne/csv.ForEachNamespace` (ingest wrapper `internal/ingestion/namespace.go`), and storage CSVs via `librobne/csv.ForEachPVC` (ingest wrapper `internal/ingestion/pvc.go`). VM / snapshot / cluster-quota CSVs still parse in `internal/ingestion`.
