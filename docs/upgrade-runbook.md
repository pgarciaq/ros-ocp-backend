...
in **000193** (#516 Path A). Keep 000110 in the chain; do not rewrite it.
...
- Rec tables can be large — see `migrations/README.md` for `CONCURRENTLY` pre-step.
...
non-active states. Populated at ingestion by [`applyNodeIdleClassification`](../librobne/node/recommend.go);
