...
The retention system runs as a background goroutine (`StartRetentionTicker`) that executes a full sweep immediately on startup and then every **24 hours**. It cleans four categories of data:
...
> disabled in Phase 1 (#258) and the tables dropped in Phase 2. Detail API plots read daily
> digests only (ADR-0292).
