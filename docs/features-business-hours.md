...
| `gpu_container_digests` | `000183_*` | `schedule_type` on the natural unique index; partition PK stays `(id, interval_start)` |
...
| Overnight window (e.g. 22:00–06:00) | Allowed on Settings PUT (`end_time < start_time`). Half-open `[start, end)` in the IANA zone; post-midnight samples belong to the previous calendar day's shift. Equal start/end is `400`. DST uses existing IANA rules (no extra math). |
...
| Overnight shifts | **Shipped** ([#488](https://github.com/pgarciaq/ros-ocp-backend/issues/488)): Settings PUT allows `end_time < start_time`. Eval was already in `InBusinessHours`. |
