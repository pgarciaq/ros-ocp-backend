...
4. Parses CSV rows into metric samples (`librobne/csv.ForEachRow` / `ForEachNamespace` / `ForEachPVC` / `ForEachVM` / `ForEachVMPVC` / `ForEachVMGPU` / `ForEachSnapshot` / `ForEachClusterQuota` via `internal/ingestion`)
