...
2. Map `ClusterCostData` → integer [`RateCard`](../../internal/engine/core/ratecard.go) **once** ([`ClusterCostDataToRateCard`](../../internal/costdata/ratecard.go)); Koku path is Tier B (namespace spend)
3. Compute savings in [`ApplySavingsEstimates()`](../../internal/engine/container/savings.go) (`RateCard` + calendar `hoursPerMonth`) and persist `estimated_savings_cents` on ingest (exposed as `estimated_savings_cents` in API JSON)
