...
CSV column headers must match koku-metrics-operator output ([`librobne/csv/csv_contract_test.go`](../../librobne/csv/csv_contract_test.go) for container ROS; [`internal/ingestion/csv_contract_test.go`](../../internal/ingestion/csv_contract_test.go) for cluster quota).
...
2. Map `ClusterCostData` → integer [`RateCard`](../../librobne/types/ratecard.go) **once** ([`ClusterCostDataToRateCard`](../../internal/costdata/ratecard.go)); Koku path is Tier B (namespace spend)
3. Compute savings in [`ApplySavingsEstimates()`](../../librobne/container/savings.go) (`RateCard` + calendar `hoursPerMonth`) and persist `estimated_savings_cents` on ingest (exposed as `estimated_savings_cents` in API JSON)
