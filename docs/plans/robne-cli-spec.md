...
| **Phase 2b — other entity CSVs** ([#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472)) | same | **(a)(b)** for node/namespace/GPU/PVC/…. Stdout. Independent of 2a. **Namespace + node/GPU + PVC + VM stdout shipped** (JSON v2/v3/v4/v5/v6 sibling arrays). quota/cluster_quota still open. **Do not close.** |
...
| **(a)(b)** other entities | More CSVs → stdout | [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472) — namespace + node/GPU + PVC + VM stdout shipped; quota/cluster_quota open. **Can run without 2a.** |
...
Container **(a)** and **(b)** (files → stdout) are shipped. Namespace, node, GPU, PVC, and VM files are a **2b** ([#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472)) slice (**stdout shipped**); quota/cluster_quota files still open. **(c)** is why PostgreSQL exists (2a, then pgdigest, 2c, 2d). The old “refuse empty DB / never migrate / already-running stack only” lock **does not serve (c)** and is **withdrawn**.
...
| **2b** | [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472) | **(a)(b)** other entity CSVs → stdout. **Namespace + node/GPU + PVC + VM stdout shipped.** quota/cluster_quota still open. **Do not close.** |
...
| **2b** ([#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472)) | Other entity CSVs | JSON / CSV / table envelopes. **Namespace + node/GPU + PVC + VM stdout shipped.** quota/cluster_quota open. |
...
No code beyond the current child until that issue is the active sprint. Remaining **2b** under [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472) (quota/cluster_quota) or **2c / [#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473)** (other entity PG). **Do not close #472** after the VM slice.
