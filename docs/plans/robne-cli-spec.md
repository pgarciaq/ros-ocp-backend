...
| **Business hours** ([#479](https://github.com/pgarciaq/ros-ocp-backend/issues/479)) | same | YAML `business_hours:` + dual digest streams (container + namespace). Not an entity CSV. Not 2b. **Shipped.** Node/GPU/VM BH is [#483](https://github.com/pgarciaq/ros-ocp-backend/issues/483). |
| **Phase 3 — diff / explain / CI** ([#480](https://github.com/pgarciaq/ros-ocp-backend/issues/480)) | same | `robne diff` (two #470 envelopes), container `robne explain` (re-run, not envelope), CI helpers. **Shipped.** Does not need Postgres. Other-entity explain is [#490](https://github.com/pgarciaq/ros-ocp-backend/issues/490). |
| **explain other entities** ([#490](https://github.com/pgarciaq/ros-ocp-backend/issues/490)) | same | Extend `robne explain` for namespace/node/GPU/PVC/VM/quota/cluster_quota/snapshot. Same subcommand. Blocked by [#480](https://github.com/pgarciaq/ros-ocp-backend/issues/480). |
...
robne explain --input <dir|file.csv|file.tar.gz|postgres://> \
  --namespace NS --workload WL --container CTR --term short --engine cost \
  [--workload-type TYPE] [--schedule all_hours|business_hours] \
  [--config] [--now] [--rate-card] [--no-user-config]
```

Re-runs the same engine path as `recommend` (files or Path A). Does **not** read a recommend envelope (hard error). Does **not** SELECT stored `expl_*`. Implicit `--plugins` is forced to `container`; an explicit other plugin is a hard error ([#490](https://github.com/pgarciaq/ros-ocp-backend/issues/490)). `--output` / `--apply-schema` / `--format` are not flags. `--schedule` defaults to `all_hours`; `business_hours` requires YAML `business_hours.enabled`. Missing match or ambiguous workload type (omit `--workload-type`) is a hard error. Stdout is one snake_case JSON object (`containerExplainOut`: identity + rec numbers + expl). Do not json-tag `ContainerExplanationFactors`. Container-only in this release.
...
**Stdout:** BH off → unchanged envelope (version still driven by plugins, including 9 for snapshot). BH on → version **10**; siblings `business_hours_recommendations` and `business_hours_namespace_recommendations` (always arrays, never `null`). Omit both keys when BH is off. Existing `recommendations` / `namespace_recommendations` stay **all_hours**. `csv` / `table`: **hard error** when `business_hours.enabled` (one entity **and** one schedule stream; JSON only).

**This PR is container and namespace only.** Node/GPU/VM BH is [#483](https://github.com/pgarciaq/ros-ocp-backend/issues/483). PVC/snapshot/quota stay out. Quota/CRQ still use **all_hours** container days.

**Postgres:** files → `--output postgres://` dual-writes container and namespace digests (`all_hours` and `business_hours`). Container recs persist all_hours only. Namespace recs persist both `schedule_type`s. Path A SELECTs matching stream(s) and never re-filters stored `all_hours`. Path A + YAML `enabled: true` + **no** `business_hours` digest rows after `pruneEmptyPathAPlugins` → **hard error** (no fallback to all_hours). Path B empty BH SELECT is OK (files semantics). Path B re-SELECTs BH container days after persist so medium/long include prior days. Namespace recs stay in-memory on Path B.
...
| **3+** ([#490](https://github.com/pgarciaq/ros-ocp-backend/issues/490)) | Same as `explain` | Other-entity `explain` DTOs / selectors |
