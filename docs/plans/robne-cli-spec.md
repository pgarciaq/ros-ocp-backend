...
**481 shipped (other-entity INSERT):** files plus `--output postgres://` also INSERT today’s other-entity daily rows via slim LWW `pgdigest.Write*` (namespace usage, node, GPU container, PVC, VM + `vm_gpu_device_digests`, namespace quota, cluster quota). Same CLI-owned DB as 463. Persist whenever the load already built those days (not plugin-gated); rec upsert stays plugin-gated ([#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473)). Quota/CRQ persist every `report_date`, not `Latest*Snapshots`. `CREATE TABLE … PARTITION OF` only for RANGE parents (`daily_namespace_digests`, `daily_node_digests`, `gpu_container_digests`, `daily_pvc_digests`); heap tables (`daily_namespace_quota_digests`, `daily_cluster_quota_digests`, `daily_vm_digests`) are not partitioned. Do not copy ingest `GREATEST`/`LEAST` (processor stream merge stays in `internal/ingestion`). Path B still computes other-entity recs from files. Other-entity digest **SELECT** / Path A is [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482). Path A still skips file-only plugins until 482.
