...
**Status:** **Greenlit** (2026-08-16). Phase 1 ([#469](https://github.com/pgarciaq/ros-ocp-backend/issues/469)), JSON envelope ([#470](https://github.com/pgarciaq/ros-ocp-backend/issues/470)), Phase **2a** ([#471](https://github.com/pgarciaq/ros-ocp-backend/issues/471)), digest INSERT ([#463](https://github.com/pgarciaq/ros-ocp-backend/issues/463)), and digest SELECT ([#474](https://github.com/pgarciaq/ros-ocp-backend/issues/474)) shipped (**container-only** for 463/474). **[#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472) 2b stdout shipped:** namespace + node/GPU + PVC + VM + quota + cluster_quota files → stdout (JSON v2–v8 sibling arrays). Remaining 2b under #472 is none. Snapshot is [#478](https://github.com/pgarciaq/ros-ocp-backend/issues/478). Business hours is [#479](https://github.com/pgarciaq/ros-ocp-backend/issues/479). Phase 3 is [#480](https://github.com/pgarciaq/ros-ocp-backend/issues/480). Other-entity digests are [#481](https://github.com/pgarciaq/ros-ocp-backend/issues/481) / [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482). **Next:** other-entity rec PG ([#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473)).  
...
| **Phase 2c — other entity PG upsert** ([#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473)) | same | **(c)** rec rows for shipped 2b plugins (incl. cluster_quota + both GPU tables). Reuse 2a `migrate.Up()` / ensure cluster. Slim `pgrec.Write*(recs)`; do not wrap GPU query-then-persist. Not digests ([#481](https://github.com/pgarciaq/ros-ocp-backend/issues/481) / [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482)). Not snapshot. |
| **Phase 2d — PG digest read** ([#474](https://github.com/pgarciaq/ros-ocp-backend/issues/474)) | same | **(c)** recompute from **this CLI’s** `daily_container_digests`. **Shipped (container-only).** Other entities: [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482). |
| **Other-entity digest INSERT** ([#481](https://github.com/pgarciaq/ros-ocp-backend/issues/481)) | same | **(c)** extend `pgdigest` for 2b `daily_*` / `gpu_container_digests`. After 473. Not rec upsert. |
| **Other-entity digest SELECT** ([#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482)) | same | **(c)** Path A for 2b plugins. After 481. Not Helm-stack SELECT. |
| **Snapshot inventory CSV** ([#478](https://github.com/pgarciaq/ros-ocp-backend/issues/478)) | same | **(a)(b)** `--plugins snapshot` files → stdout. Not 2b. |
| **Business hours** ([#479](https://github.com/pgarciaq/ros-ocp-backend/issues/479)) | same | YAML `business_hours:` + digest filtering. Not an entity CSV. Not 2b. |
| **Phase 3 — diff / explain / CI** ([#480](https://github.com/pgarciaq/ros-ocp-backend/issues/480)) | same | `robne diff`, `robne explain`, CI helpers. **(a)(b)** can start after [#470](https://github.com/pgarciaq/ros-ocp-backend/issues/470); does not need Postgres. |
...
| **(a)(b)** goldens / compare | `diff` on JSON envelopes | [#480](https://github.com/pgarciaq/ros-ocp-backend/issues/480) — **does not wait on Postgres** |
...
| **(c)** keep other entity recs | Same persist, other tables | [#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473) after 471+472. Recs only; Path A still skips those plugins until [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482). |
| **(c)** keep daily usage (medium/long terms, charts, 2d) | Digest **INSERT** | Containers: [#463](https://github.com/pgarciaq/ros-ocp-backend/issues/463) **shipped.** Other entities: [#481](https://github.com/pgarciaq/ros-ocp-backend/issues/481). Daily operator payloads are ~one day; without stored digests, medium/long windows collapse. |
| **(c)** recompute without re-reading CSV | Digest **SELECT** | Containers: [#474](https://github.com/pgarciaq/ros-ocp-backend/issues/474) **shipped.** Other entities: [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482). Not “read a Helm ROS DB.” |
...
Phase 1 (container files → stdout) is **shipped**. Phase **2b** files → stdout is **shipped**. Still open for **(a)(b):** Phase 3 `diff` ([#480](https://github.com/pgarciaq/ros-ocp-backend/issues/480)). Still open for **(c):** 2c ([#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473)), then other-entity digests ([#481](https://github.com/pgarciaq/ros-ocp-backend/issues/481) / [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482)).
...
**`--plugins`:** comma-separated allowlist (`container`, `namespace`, `node`, `gpu`, `pvc`, `vm`, `quota`, `cluster_quota`). Same idea as “enable recommenders,” **not** `internal/plugins` `init()` registration. Default is `container`. Those names are accepted when listed in YAML or `--plugins`. Other names still error until that plugin’s issue lands ([#478](https://github.com/pgarciaq/ros-ocp-backend/issues/478) for `snapshot`). Node and GPU still read **container ROS CSV** (no new file family). PVC reads a **storage** file family (`ros-openshift-storage-*`, `ocp_storage_usage`, `cm-openshift-storage-usage` classified **before** blanket `cm-openshift-*` → cost-only). VM reads **usage** (`ros-openshift-vm-usage-*`, `ocp_ros_vm_usage` classified **before** `ocp_ros_usage`) plus optional companions (`ros-openshift-vm-pvc-*` / `ocp_ros_vm_pvc`, `ros-openshift-vm-gpu-device-*` / `ocp_ros_vm_gpu_device`; missing/malformed companions degrade). Quota reads the **same namespace ROS CSV** as `--plugins namespace` (optional `quota_name` / `resource_quota_name`; named-quota `*_namespace_sum` columns are ResourceQuota hard). Cluster quota reads a **dedicated CRQ** file family (`ros-openshift-cluster-quota-*`, `ocp_ros_cluster_quota` classified **before** namespace). YAML `node:` / `gpu:` / `pvc:` / `vm:` / `quota:` / `cluster_quota:` stay reserved (compiled defaults; quota/CRQ headroom 11000 BP). Do **not** treat GPU timeslicing as a model for quota: namespace ResourceQuota (`quota`) and ClusterResourceQuota (`cluster_quota`) are separate plugins.
...
| Rule | Detail |
|------|--------|
| Envelope | `version` (int: `1` container-only, `2` namespace, `3` node, `4` gpu, `5` pvc, `6` vm, `7` quota, `8` cluster_quota — **max** of enabled plugins), `cluster_id`, `now` (RFC3339 UTC), `skipped_rows`, `recommendations` (always an array, never `null`; **container rows only**) |
...
**PVC sibling (same #472 slice):** `--plugins pvc` bumps `version` to at least **5** and adds `pvc_recommendations` (always an array, never `null`). One rec per PVC×term (no `engine` column). Missing optional columns (`vm_name`, usage, request) do not fail; empty PVC name rows are dropped. `--plugins pvc` with no storage CSV is an error. Default `--plugins` ignores storage files. Do not call `ApplyPVCSavings` (unset savings stay JSON `null`). YAML `pvc:` stays reserved. Do **not** add `json` tags on `pvc.PVCRec`.

**VM sibling (same #472 slice):** `--plugins vm` bumps `version` to at least **6** and adds `vm_recommendations` (always an array, never `null`). One rec per VM×term×engine (`cost`, `performance`). Overlay container `terms` apply via `VMTermWindowsFromConfig` until YAML `vm:` unlocks (CLI short term is 1 day / min 1, not product VM 7/15/30). Missing companion files degrade; malformed companions in a dir/tarball are skipped; `--plugins vm` with no **usage** CSV is an error. Empty `vm_name` / `namespace` rows are skipped. Timeslicing is **on the VM row** (`recommended_time_slice_count`), not a second sibling (GPU’s node×model timeslicing array is a different grain). Do not call product VM savings (unset `estimated_savings_cents` stays JSON `null`). YAML `vm:` stays reserved. Do **not** add CLI `json` tags on `vm.VMRecommendation` (existing GPU field tags in librobne stay). Cluster quota is **8**. Snapshot is [#478](https://github.com/pgarciaq/ros-ocp-backend/issues/478) (not 2b).

**Quota sibling (same #472 slice):** `--plugins quota` bumps `version` to at least **7** and adds `quota_recommendations` (always an array, never `null`). No new Kind — same namespace ROS CSV as `--plugins namespace`. Rows without `quota_name` (alias `resource_quota_name`) are skipped as snapshots (empty sibling, not an error). Missing namespace CSV is an error. One rec per namespace×quota_name (no term/engine). Named-quota operator `*_namespace_sum` columns are ResourceQuota hard. If container ROS is in the load, compute in-memory container recs (term `medium`, engine `cost`) for `ContainerQuotaAggregate` even when `--plugins container` is off; missing container ROS yields zero aggregates. Compiled defaults: 10% headroom = 11000 BP, high 9000, medium 7000, empty currency (`DefaultQuotaRecConfig`). Do not call `ApplyQuotaSavings` (unset savings stay JSON `null`). YAML `quota:` stays reserved. Do **not** add `json` tags on `quota.QuotaRec`. `--output postgres://` still persists containers only. `--input postgres://` skips quota or errors if it is the only plugin.

**Cluster quota sibling (same #472 slice):** `--plugins cluster_quota` bumps `version` to at least **8** and adds `cluster_quota_recommendations` (always an array, never `null`). Dedicated Kind (`KindClusterQuota`): classify `ros-openshift-cluster-quota-` and `ocp_ros_cluster_quota` **before** namespace. Missing CRQ CSV with the plugin on is an error. Default `--plugins` ignores a CRQ file in the dir. Days with no hard limits emit an empty sibling (not an error). Empty `namespaces` on the snapshot sums **all** in-memory namespace quota recs (product `QueryNamespaceQuotaAggregateForNamespaces`); a non-empty comma-separated list filters by membership. Two ResourceQuotas in one namespace both count. Missing NS/container ROS still emits CRQ recs from used vs hard (zero nsAggs). Nested chain: if container ROS is present, compute in-memory container recs (`medium`/`cost`); if namespace ROS is present, compute quota recs; sum into `NamespaceQuotaClusterAggregate` per CRQ; then `RecommendClusterQuotas`. Attach quota before cluster_quota so `--plugins quota` can reuse `QuotaRecs`. Do **not** emit `quota_recommendations` unless `--plugins quota` is on. DTO (`clusterQuotaOut`) matches `quotaOut` grain: hard + recommended CPU millicores, memory/storage **bytes**, pods; omit used, utilization, notification codes, capacity freed, object_count, Expl. Do not convert CRQ memory to KiB. Do not call `ApplyClusterQuotaSavings` (unset savings stay JSON `null`). YAML `cluster_quota:` stays reserved (`enabled: true` is still a Phase-1 error). Do **not** add `json` tags on `quota.ClusterQuotaRec`. `--output postgres://` still persists containers only. `--input postgres://` skips cluster_quota or errors if it is the only plugin. Snapshot is [#478](https://github.com/pgarciaq/ros-ocp-backend/issues/478) (not 2b).
...
| **2b** | [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472) | **(a)(b)** other entity CSVs → stdout. **Shipped.** Snapshot is [#478](https://github.com/pgarciaq/ros-ocp-backend/issues/478). Business hours is [#479](https://github.com/pgarciaq/ros-ocp-backend/issues/479). |
| **2c** | [#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473) | **(c)** other entity **rec** upsert (same migrate/ensure as 2a). Not digests. |
| **2d** | [#474](https://github.com/pgarciaq/ros-ocp-backend/issues/474) | **(c)** `SELECT` **container** digests this CLI wrote. **Shipped.** |
| **2b digest INSERT** | [#481](https://github.com/pgarciaq/ros-ocp-backend/issues/481) | **(c)** other-entity digest INSERT. After 473. |
| **2b digest SELECT** | [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482) | **(c)** Path A for 2b plugins. After 481. |

**2d shipped (containers):** files plus `--output postgres://` INSERT today’s `all_hours` **container** digests, commit, then SELECT `[end − MaxWindowDays(terms), end]` (`end` is `--now` if set, else `max(bucket_date)` — never wall clock, never `ROS_MAX_LOOKBACK_DAYS`), then `RecommendWorkloads` and rec upsert. `--input postgres://` (or `postgresql://`) is recompute: same SELECT, stdout, optional rec upsert if `--output` is the **same** database; `--apply-schema` is a hard error. `validate` stays files-only. Empty SELECT is a CLI error. Processor timeout/cap stay in the wrapper. `#476` can later point `QueryContainerDigests*` at the same `pgdigest` Read. Other-entity digest INSERT/SELECT is [#481](https://github.com/pgarciaq/ros-ocp-backend/issues/481) / [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482). Path A still skips file-only plugins until 482.
...
`librobne/csv` parses **container** ROS, **namespace** ROS (`KindNamespace`: NISE `*ocp_ros_namespace_usage.csv`, operator `ros-openshift-namespace-*.csv`), **storage** (`KindStorage`: `ros-openshift-storage-*`, `ocp_storage_usage`, `cm-openshift-storage-usage` before blanket `cm-openshift-*` → cost-only), **VM** (`KindVM` / `KindVMPVC` / `KindVMGPU`: `ros-openshift-vm-usage-*` / `ocp_ros_vm_usage`, optional `ros-openshift-vm-pvc-*` / `ocp_ros_vm_pvc`, `ros-openshift-vm-gpu-device-*` / `ocp_ros_vm_gpu_device`), and **cluster quota** (`KindClusterQuota`: `ros-openshift-cluster-quota-*`, `ocp_ros_cluster_quota` classified **before** namespace). Classify VM usage **before** `ocp_ros_usage` (`ocp_ros_vm_usage` contains that substring). Classify namespace **before** `ocp_ros_usage` so the substring does not steal namespace files. Container ROS optional columns feed in-memory node and GPU daily aggregation (`DailyNodeDigests`, `DailyGPUDigests`). Storage rows feed `DailyPVCDigests` (byte-seconds → bytes, same as ingest `ComputePVCDigests`). VM usage + companions feed `DailyVMDigests` (companions attach onto existing usage days only). Namespace ROS optional quota columns feed `LatestNamespaceQuotaSnapshots` (skip empty `quota_name`; daily max then latest day per namespace×quota_name). Cluster-quota rows feed `LatestClusterQuotaSnapshots` (daily max, last non-empty namespaces, latest day **with hard limits**, sort by name). `internal/ingestion` already parses those families. Remaining **2b** under [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472) is none. Do **not** rewrite ingest as a 2a/2b/pgdigest side quest. Later ingest can call the librobne parsers ([#475](https://github.com/pgarciaq/ros-ocp-backend/issues/475)). Operator never imports `csv`.

### Reserved YAML keys until 2b

`business_hours`, `node`, `gpu`, `pvc`, `vm`, `quota`, `cluster_quota` error if `enabled: true` (`reservedYAMLKeys`). **Keep those YAML-block errors** even after `--plugins node` / `gpu` / `pvc` / `vm` / `quota` / `cluster_quota` unlock (this slice uses compiled defaults; overlay container `terms` for VM). Unlock YAML **per entity in the same PR** that parses a dedicated settings schema. Namespace has **no** reserved `namespace:` YAML block — it reuses container `sizing` / `terms`. `--plugins namespace`, `node`, `gpu`, `pvc`, `vm`, `quota`, and `cluster_quota` are unlocked. `--plugins snapshot` is [#478](https://github.com/pgarciaq/ros-ocp-backend/issues/478). `business_hours:` is [#479](https://github.com/pgarciaq/ros-ocp-backend/issues/479) (not a `--plugins` name).
...
**Not Phase 1. Not [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472).** Child: [#479](https://github.com/pgarciaq/ros-ocp-backend/issues/479).

Digest BH filtering is YAML `business_hours.enabled` plus `bhschedule` — not an entity CSV. Do not wire `bhschedule` until that issue is the active sprint. Namespace / other 2b plugins do **not** unlock `business_hours:`.
...
| **2c** ([#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473)) | 2b files | Other entity **rec** PG upsert |
...
| **2b digest INSERT** ([#481](https://github.com/pgarciaq/ros-ocp-backend/issues/481)) | 2b files | Other-entity digest INSERT (`pgdigest` extend). After 473. |
| **2b digest SELECT** ([#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482)) | This CLI’s other-entity digest tables | Path A for 2b plugins. After 481. |
| **snapshot** ([#478](https://github.com/pgarciaq/ros-ocp-backend/issues/478)) | Snapshot inventory CSV | JSON / CSV / table. Not 2b. |
| **business hours** ([#479](https://github.com/pgarciaq/ros-ocp-backend/issues/479)) | YAML `business_hours:` | Digest filtering. Not an entity CSV. |
| **3** ([#480](https://github.com/pgarciaq/ros-ocp-backend/issues/480)) | Two JSON envelopes (from (a)(b) or (c)) | `diff`, `explain`, CI |

`librobne/csv` landed in Phase 1. **pgdigest** INSERT is [#463](https://github.com/pgarciaq/ros-ocp-backend/issues/463), **shipped (containers).** Digest **SELECT** (recommend path) is 2d ([#474](https://github.com/pgarciaq/ros-ocp-backend/issues/474)), **shipped (containers).** Other-entity digests are [#481](https://github.com/pgarciaq/ros-ocp-backend/issues/481) / [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482). Ingest parser dedup ([#475](https://github.com/pgarciaq/ros-ocp-backend/issues/475)) and `QueryContainerDigests*` dedup ([#476](https://github.com/pgarciaq/ros-ocp-backend/issues/476)) are P5 under [#94](https://github.com/pgarciaq/ros-ocp-backend/issues/94), not CLI children.
...
8. Business hours not Phase 1 (§7). Child: [#479](https://github.com/pgarciaq/ros-ocp-backend/issues/479). Namespace plugin does **not** unlock `business_hours:`.
9. Fix koku `./` matching in koku ([#466](https://github.com/pgarciaq/ros-ocp-backend/issues/466)); CLI already normalizes (§8).

No code beyond the current child until that issue is the active sprint. Remaining **2b** under [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472) is none. Snapshot is [#478](https://github.com/pgarciaq/ros-ocp-backend/issues/478). Business hours is [#479](https://github.com/pgarciaq/ros-ocp-backend/issues/479). Other-entity digests are [#481](https://github.com/pgarciaq/ros-ocp-backend/issues/481) / [#482](https://github.com/pgarciaq/ros-ocp-backend/issues/482). **Next:** **2c / [#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473)** (other entity rec PG).
