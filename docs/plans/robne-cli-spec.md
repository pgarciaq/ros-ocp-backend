...
**Related, not a #99 child:** ingest parser dedup ([#475](https://github.com/pgarciaq/ros-ocp-backend/issues/475), **shipped** — container `ForEachRow`; [#501](https://github.com/pgarciaq/ros-ocp-backend/issues/501) **shipped** — namespace `ForEachNamespace` + PVC `ForEachPVC` + VM `ForEachVM` + VM sidecar `ForEachVMPVC` / `ForEachVMGPU` + snapshot `ForEachSnapshot` + cluster-quota `ForEachClusterQuota`) and digest-read dedup ([#476](https://github.com/pgarciaq/ros-ocp-backend/issues/476), **shipped** — `QueryContainerDigests*` → `pgdigest.ForEachSchedule*`) are P5 processor hygiene under [#94](https://github.com/pgarciaq/ros-ocp-backend/issues/94). They do not change `bin/robne`.
...
org_id: "1234567"                 # required for PostgreSQL write (Phase 2a); optional in Phase 1. Must match existing PVC rows for that cluster_uuid (#508).
...
**2d shipped (containers):** files plus `--output postgres://` INSERT today’s `all_hours` **container** digests, commit, then SELECT `[end − MaxWindowDays(terms), end]` (`end` is `--now` if set, else `max(bucket_date)` — never wall clock, never `ROS_MAX_LOOKBACK_DAYS`), then `RecommendWorkloads` and rec upsert. `--input postgres://` (or `postgresql://`) is recompute: same SELECT, stdout, optional rec upsert if `--output` is the **same** database; `--apply-schema` is a hard error. `validate` stays files-only. Empty SELECT is a CLI error. Processor timeout/cap stay in the wrapper. `#476` shipped: `QueryContainerDigests*` streams `pgdigest.ForEachSchedule*` (page `unnest` unchanged).

**481 shipped (other-entity INSERT):** files plus `--output postgres://` also INSERT today’s other-entity daily rows via slim `pgdigest.Write*` (namespace usage, node, GPU container, PVC, VM + `vm_gpu_device_digests`, namespace quota, cluster quota). Same CLI-owned DB as 463. Persist whenever the load already built those days (not plugin-gated); rec upsert stays plugin-gated ([#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473)). Quota/CRQ persist every `report_date`, not `Latest*Snapshots`. `CREATE TABLE … PARTITION OF` only for RANGE parents (`daily_namespace_digests`, `daily_node_digests`, `gpu_container_digests`, `daily_pvc_digests`); heap tables (`daily_namespace_quota_digests`, `daily_cluster_quota_digests`, `daily_vm_digests`) are not partitioned. **[#505](https://github.com/pgarciaq/ros-ocp-backend/issues/505):** PVC and quota writers copy ingest `GREATEST`/`LEAST` (weighted PVC avg, `sample_count+=`, empty pod/VM/namespaces keep existing; PVC conflict does not rewrite `org_id`). **[#508](https://github.com/pgarciaq/ros-ocp-backend/issues/508):** YAML `org_id` must match existing PVC rows for that `cluster_uuid`. Unique key still omits `org_id`; a mismatched tenant still merges GREATEST/LEAST into the original row. Path A SELECT filters `org_id`. GPU schema unchanged. Container, namespace usage, node, GPU, and VM stay last-write-wins. A second `--output` of the same full PVC day doubles `sample_count`; a thinner day cannot shrink that day’s extrema. Path B still computes other-entity recs from files.
