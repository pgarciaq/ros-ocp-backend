...
| **Phase 2b — other entity CSVs** ([#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472)) | same | **(a)(b)** for node/namespace/GPU/PVC/…. Stdout. Independent of 2a. **Namespace + node/GPU + PVC stdout shipped** (JSON v2/v3/v4/v5 sibling arrays). VM/quota/cluster_quota still open. **Do not close.** |
...
| **(a)(b)** other entities | More CSVs → stdout | [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472) — namespace + node/GPU + PVC stdout shipped; VM/quota/cluster_quota open. **Can run without 2a.** |
...
Container **(a)** and **(b)** (files → stdout) are shipped. Namespace, node, GPU, and PVC files are a **2b** ([#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472)) slice (**stdout shipped**); VM/quota/cluster_quota files still open. **(c)** is why PostgreSQL exists (2a, then pgdigest, 2c, 2d). The old “refuse empty DB / never migrate / already-running stack only” lock **does not serve (c)** and is **withdrawn**.
...
**`--plugins`:** comma-separated allowlist (`container`, `namespace`, `node`, `gpu`, `pvc`; later `vm`, `quota`, `cluster_quota`, …). Same idea as “enable recommenders,” **not** `internal/plugins` `init()` registration. Default is `container`. `namespace`, `node`, `gpu`, and `pvc` are accepted when listed in YAML or `--plugins`. Other names still error until that entity’s CSV parse lands in the same PR. Node and GPU still read **container ROS CSV** (no new file family). PVC reads a **storage** file family (`ros-openshift-storage-*`, `ocp_storage_usage`, `cm-openshift-storage-usage` classified **before** blanket `cm-openshift-*` → cost-only). YAML `node:` / `gpu:` / `pvc:` stay reserved (compiled defaults). Do **not** treat GPU timeslicing as a model for quota: namespace ResourceQuota and ClusterResourceQuota are separate plugins (`quota` vs `cluster_quota`) when those slices land. Do not pre-allocate a snapshot version.
...
| **2b** | [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472) | **(a)(b)** other entity CSVs → stdout. **Namespace + node/GPU + PVC stdout shipped.** VM/quota/cluster_quota still open. **Do not close.** |
...
`librobne/csv` parses **container** ROS, **namespace** ROS (`KindNamespace`: NISE `*ocp_ros_namespace_usage.csv`, operator `ros-openshift-namespace-*.csv`), and **storage** (`KindStorage`: `ros-openshift-storage-*`, `ocp_storage_usage`, `cm-openshift-storage-usage` before blanket `cm-openshift-*` → cost-only). Classify namespace **before** `ocp_ros_usage` so the substring does not steal namespace files. Container ROS optional columns feed in-memory node and GPU daily aggregation (`DailyNodeDigests`, `DailyGPUDigests`). Storage rows feed `DailyPVCDigests` (byte-seconds → bytes, same as ingest `ComputePVCDigests`). VM/quota/cluster_quota files remain skipped until their 2b slice. `internal/ingestion` already parses those families. Remaining **2b** ([#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472)) grows `librobne/csv` for those files → stdout. Do **not** rewrite ingest as a 2a/2b/pgdigest side quest. Later ingest can call the librobne parsers ([#475](https://github.com/pgarciaq/ros-ocp-backend/issues/475)). Operator never imports `csv`.

### Reserved YAML keys until 2b

`business_hours`, `node`, `gpu`, `pvc`, `vm`, `quota` error if `enabled: true` (`reservedYAMLKeys`). **Keep those YAML-block errors** even after `--plugins node` / `gpu` / `pvc` unlock (this slice uses compiled defaults). Unlock YAML **per entity in the same PR** that parses a dedicated settings schema. Namespace has **no** reserved `namespace:` YAML block — it reuses container `sizing` / `terms`. `--plugins namespace`, `node`, `gpu`, and `pvc` are unlocked. Do not add `cluster_quota` to reserved YAML until that plugin unlocks. Do not pre-unlock empty stubs for VM/quota.
...
| **2b** ([#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472)) | Other entity CSVs | JSON / CSV / table envelopes. **Namespace + node/GPU + PVC stdout shipped.** VM/quota/cluster_quota open. |
...
No code beyond the current child until that issue is the active sprint. Remaining **2b** under [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472) (VM/quota/cluster_quota) or **2c / [#473](https://github.com/pgarciaq/ros-ocp-backend/issues/473)** (other entity PG). **Do not close #472** after the PVC slice.
