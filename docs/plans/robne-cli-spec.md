...
| **Business hours** ([#479](https://github.com/pgarciaq/ros-ocp-backend/issues/479)) | same | YAML `business_hours:` + dual digest streams (container + namespace). Not an entity CSV. Not 2b. **Shipped.** Product node API ([#484](https://github.com/pgarciaq/ros-ocp-backend/issues/484)) shipped (no CLI JSON siblings). CLI JSON node/GPU/VM BH is [#487](https://github.com/pgarciaq/ros-ocp-backend/issues/487). GPU product API ([#485](https://github.com/pgarciaq/ros-ocp-backend/issues/485)) shipped (no CLI JSON siblings). GPU timeslicing product BH ([#491](https://github.com/pgarciaq/ros-ocp-backend/issues/491)) shipped (detail nest only; YAML `business_hours` + `--plugins gpu` remains a hard error). VM product is [#486](https://github.com/pgarciaq/ros-ocp-backend/issues/486). |
...
**One entity type per run ([#490](https://github.com/pgarciaq/ros-ocp-backend/issues/490) shipped).** `--plugins` is **exactly one** name. Omit it for **container**. Two or more names is a hard error. YAML `plugins:` does **not** select the type (unlike `recommend`). No `--entity` flag. Inapplicable identity flags are hard errors (validated in `runExplain`, not cobra `MarkFlagRequired`). `--schedule` defaults to `all_hours`; `business_hours` requires YAML `business_hours.enabled` and is **container and namespace only** — other plugins hard-error and point at [#487](https://github.com/pgarciaq/ros-ocp-backend/issues/487). Snapshot + Path A stays the existing hard error.
