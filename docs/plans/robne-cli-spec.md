...
**Stdout:** BH off → unchanged envelope (version still driven by plugins, including 9 for snapshot). BH on → version **10** for container/namespace siblings; version **11** when `--plugins` includes node, gpu, or vm (or implicit default keeps node). Siblings `business_hours_recommendations` and `business_hours_namespace_recommendations` (always arrays, never `null`). Node/GPU/timeslicing/VM BH keys (`business_hours_node_recommendations`, `business_hours_gpu_recommendations`, `business_hours_gpu_timeslicing_recommendations`, `business_hours_vm_recommendations`) are **full CLI DTOs** (same shape as all-hours siblings) — **not** the product thin nest. Omit a key when that plugin is off. Empty after weighting is still `[]`, never copy all-hours into a BH key. Omit all BH keys when BH is off. Existing `recommendations` / `namespace_recommendations` stay **all_hours**. `csv` / `table`: **hard error** when `business_hours.enabled` (one entity **and** one schedule stream; JSON only).
