...
**One entity type per run ([#490](https://github.com/pgarciaq/ros-ocp-backend/issues/490) shipped).** `--plugins` is **exactly one** name. Omit it for **container**. Two or more names is a hard error. YAML `plugins:` does **not** select the type (unlike `recommend`). No `--entity` flag. Inapplicable identity flags are hard errors (validated in `runExplain`, not cobra `MarkFlagRequired`). `--schedule` defaults to `all_hours`; `business_hours` requires YAML `business_hours.enabled` and is **container and namespace only** — other plugins hard-error and point at [#483](https://github.com/pgarciaq/ros-ocp-backend/issues/483). Snapshot + Path A stays the existing hard error.
