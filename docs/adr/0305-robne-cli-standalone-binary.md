...
Current: in-tree `cmd/robne` (Phase 1+2a+container pgdigest INSERT/SELECT+namespace/node/gpu/pvc/vm/quota/cluster_quota stdout+2c other-entity rec upsert+other-entity digest INSERT shipped). Later: optional split to a `robne-cli` repo.
