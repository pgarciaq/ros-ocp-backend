...
Current: in-tree `cmd/robne` (Phase 1+2a+pgdigest INSERT+SELECT+namespace/node/gpu/pvc stdout shipped). Later: optional split to a `robne-cli` repo.
...
and **2b namespace + node/GPU + PVC files → stdout** (`--plugins namespace|node|gpu|pvc`, JSON v2/v3/v4/v5 sibling arrays; [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472)
stays open for VM/quota/cluster_quota) shipped. It imports librobne **plus** optional I/O
