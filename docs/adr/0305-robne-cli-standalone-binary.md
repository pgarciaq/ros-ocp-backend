...
Current: in-tree `cmd/robne` (Phase 1+2a+pgdigest INSERT+SELECT+namespace/node/gpu stdout shipped). Later: optional split to a `robne-cli` repo.
...
and **2b namespace + node/GPU files → stdout** (`--plugins namespace|node|gpu`, JSON v2/v3/v4 sibling arrays; [#472](https://github.com/pgarciaq/ros-ocp-backend/issues/472)
stays open for PVC/VM/quota) shipped. It imports librobne **plus** optional I/O
