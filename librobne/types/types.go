...
// KeyedDigest is one digest row plus its container identity. Consumers scan or
// parse directly into this type. RecommendWorkloads does not copy the backing array.
type KeyedDigest struct {
	Key ContainerKey
	Row DigestRow
}

// EmitContainer is called with a reused batch of container recommendations.
// The caller must copy if it retains the slice after return.
type EmitContainer func(batch []ContainerRec) error

// EngineConfig is deposited, pool-free config for RecommendWorkloads.
// Wrappers load terms/thresholds/idle from the product and pass values here.
// Now and StalenessThreshold must be set by the caller (no time.Now inside the runner).
