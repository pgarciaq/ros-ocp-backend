...
// VMFBUsedFraction returns the fraction of GPU frame buffer used by a VM device.
func VMFBUsedFraction(model string, fbUsedMaxMiB, fbUsedAvgMiB float64) float64 {
	spec := MatchGPUModel(model)
	if spec == nil || spec.TotalFBMiB <= 0 {
		return 0
	}
	fb := fbUsedMaxMiB
	if fb <= 0 && fbUsedAvgMiB > 0 {
		fb = fbUsedAvgMiB
	}
	if fb <= 0 {
		return 0
