...
const (
	...
	NotifGPUTimeSharingCandidate = types.NotifGPUTimeSharingCandidate
...
type TimeslicingRec struct {
	...
	Expl                types.NodeGPUTimeslicingExplanationFactors
}
...
func ComputeNodeTimeslicingRecWithSettings(group NodeGPUGroup, gpuRate *float32, now time.Time, settings GPUThresholdSettings) *TimeslicingRec {
	...
	rec := &TimeslicingRec{
		...
		NotificationCodes:   []int16{types.NotifGPUTimeSharingCandidate},
		Expl: types.NodeGPUTimeslicingExplanationFactors{
	...
	for _, c := range candidates {
		...
		c.Rec.NotificationCodes = append(c.Rec.NotificationCodes, types.NotifGPUTimeSharingCandidate)
		if perGPU != nil {
			savings := *perGPU
