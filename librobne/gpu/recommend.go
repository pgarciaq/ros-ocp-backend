...
func RecommendGPUWithSettings(digests []GPUDigestRow, settings GPUThresholdSettings, idleCfg ...GPUIdleConfig) *GPURec {
	...
	if !hasProf {
		...
		rec.NotificationCodes = append(rec.NotificationCodes, types.NotifGPUNoProfilingData)
		if isMultiGPU {
			rec.NotificationCodes = append(rec.NotificationCodes, types.NotifGPUMultiDevice)
	...
	case GPUClassIdle:
		rec.NotificationCodes = append(rec.NotificationCodes, types.NotifGPUIdle)
	case GPUClassUnderutilized, GPUClassComputeBoundUnderutil:
		rec.NotificationCodes = append(rec.NotificationCodes, types.NotifGPUUnderutilized)
	case GPUClassMemoryBound:
		rec.MemoryBoundDetected = true
		rec.NotificationCodes = append(rec.NotificationCodes, types.NotifGPUMemBound)
	}
...
// Unexported aliases keep product query helpers and tests calling the original names.
var filterGPUByWindow = FilterGPUByWindow
var latestGPUDigest = LatestGPUDigest
