...
func makeDigestRowWithType(node, instanceType string, day int, cpuP50, cpuP95, memP50, memP95, cpuReqs, memReqs int64, allocCPU, allocMem *int64) DigestRow {
	d := makeDigestRow(node, day, cpuP50, cpuP95, memP50, memP95, cpuReqs, memReqs, allocCPU, allocMem)
	d.InstanceType = instanceType
	return d
}

func underutilizedNodeDigests(node, instanceType string, days int, cpuP95, memP95 int64) []DigestRow {
	allocCPU := ptr64(16000)
	allocMem := ptr64(65536)
	digests := make([]DigestRow, days)
	for i := 0; i < days; i++ {
		day := i + 1
...
func relaxedUnderutilConfig() RecConfig {
	cfg := defaultRecConfig()
	cfg.UnderutilThresholdBP = fixedpoint.FloatToBasisPoints(0.40)
	return cfg
}

func recsByNode(recs []Rec, term, engine string) map[string]Rec {
...
func totalReduction(recs []Rec, term, engine string) int {
	sum := 0
	for _, r := range recs {
		if r.Term == term && r.Engine == engine {
